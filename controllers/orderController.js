...
export async function createOrder(req, res) {
	...
	try {
		...
		let newOrderID = `${ORDER_ID_PREFIX}0000001`;

		if (orderList.length != 0) {
			let lastOrderIDInString = orderList[0].orderID;
			let lastOrderNumberMatch = lastOrderIDInString.match(/(\d+)$/);
			let lastOrderNumber = lastOrderNumberMatch ? parseInt(lastOrderNumberMatch[1]) : 0;
			let newOrderNumber = lastOrderNumber + 1;
			//padStart
			let newOrderNumberInString = newOrderNumber.toString().padStart(7, "0");

			newOrderID = ORDER_ID_PREFIX + newOrderNumberInString;
		}
